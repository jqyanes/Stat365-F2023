...
* SIGNED: [YOUR RUID ONLY - DO NOT WRITE YOUR NAME] 209008173
...
```{r}
the_sim <- stan(model_code = the_model, data = list(Y = c(3,4)), chains = 4, iter = 10000, seed=1000)
```
...
* Is this result satisfactory? [YOUR ANSWER: YES] 
...
one_iteration <- function(a, b, current){
 ...
 proposal_plaus <- dbeta(proposal, 1, 2) * dbinom(c(3,4), 6, proposal)
 proposal_q     <- dbeta(proposal, a, b)
 current_plaus  <- dbeta(current, 1, 2) * dbinom(c(3,4), 6, current)
